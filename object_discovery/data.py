...
class TetrominoesDataset(Dataset):
    def __init__(
        ...
        if self.max_n_objects:
            if self.split == "train":
                num_objects_in_scene = np.sum(self.data["visibility"][:60_000], axis=1)
            elif self.split == "val":
                num_objects_in_scene = np.sum(
                    self.data["visibility"][60_001:75_000], axis=1
                )
            elif self.split == "test":
                num_objects_in_scene = np.sum(
...
class TetrominoesDataModule(pl.LightningDataModule):
    def __init__(
        self,
        data_root: str,
        train_batch_size: int,
        ...
        num_workers: int,
        resolution: Tuple[int, int],
        tetrominoes_transforms: Optional[Callable] = None,
        mask_transforms: Optional[Callable] = None,
        neg_1_to_pos_1_scale: bool = True,
    ):
        super().__init__()
        ...
        self.resolution = resolution
        self.neg_1_to_pos_1_scale = neg_1_to_pos_1_scale
        self.max_num_entries = 4  # 3 pieces and the background

        print(
            f"INFO: limiting the dataset to only images with `max_n_objects` ({max_n_objects}) objects."
        )

        if not self.tetrominoes_transforms:
            current_transforms = [
                # image has shape (H x W x C)
                transforms.ToTensor(),  # rescales to range [0.0, 1.0]
                # image has shape (C x H x W)
            ...
            self.tetrominoes_transforms = transforms.Compose(current_transforms)
        ...
        self.train_dataset = TetrominoesDataset(
            data_root=self.data_root,
            tetrominoes_transforms=self.tetrominoes_transforms,
            mask_transforms=self.mask_transforms,
            split="train",
            max_n_objects=self.max_n_objects,
        )
        self.val_dataset = TetrominoesDataset(
            data_root=self.data_root,
            tetrominoes_transforms=self.tetrominoes_transforms,
            mask_transforms=self.mask_transforms,
            split="val",
